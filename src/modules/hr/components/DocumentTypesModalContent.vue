<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h3 class="text-2xl font-bold text-gray-900">Document Types</h3>
        <p class="mt-1 text-sm text-gray-600">Manage document types and their rules</p>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      <div class="bg-gray-50 rounded-lg border border-gray-200 p-4">
        <div class="flex items-center">
          ...
          <div class="ml-3">
            <p class="text-xs font-medium text-gray-500">Total Types</p>
      ...
      <div class="bg-gray-50 rounded-lg border border-gray-200 p-4">
        <div class="flex items-center">
          ...
          <div class="ml-3">
            <p class="text-xs font-medium text-gray-500">Mandatory</p>
            <p class="text-xl font-bold text-gray-900">{{ stats.mandatory }}</p>
          </div>
        </div>
      </div>
      <div class="bg-gray-50 rounded-lg border border-gray-200 p-4">
        <div class="flex items-center">
          ...
          <div class="ml-3">
            <p class="text-xs font-medium text-gray-500">Employee Upload</p>
            <p class="text-xl font-bold text-gray-900">{{ stats.employeeUpload }}</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
        <div>
          ...
          <input
            ...
            class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-purple focus:border-transparent"
            @input="handleSearch"
          />
        </div>
        <div>
        ...
        <div>
          <label class="block text-xs font-medium text-gray-700 mb-1">Mandatory</label>
          <select
    ...
    <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
      <div class="px-4 py-3 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
        <h4 class="text-sm font-semibold text-gray-900">Document Types</h4>
        <button
          @click="showCreateModal = true"
          class="inline-flex items-center px-3 py-1.5 text-xs font-medium rounded-lg text-white bg-primary-purple hover:bg-primary-pink transition-colors"
        >
      ...
      <div class="p-4">
        <div v-if="documentTypesStore.loading" class="text-center py-8">
          <div class="animate-spin rounded-full h-8 w-8 border-4 border-primary-purple border-t-transparent mx-auto"></div>
          <p class="mt-2 text-sm text-gray-600">Loading document types...</p>
        </div>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Mandatory</th>
                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Employee Upload</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="docType in documentTypesStore.documentTypes" :key="docType.id" class="hover:bg-gray-50">
                <td class="px-4 py-3">
                  ...
                  <div v-if="docType.description" class="text-xs text-gray-500 mt-0.5">{{ docType.description }}</div>
                </td>
                <td class="px-4 py-3 text-sm text-gray-500">{{ docType.code || '-' }}</td>
                <td class="px-4 py-3">
                ...
                <td class="px-4 py-3 text-sm font-medium">
                  <button
                    ...
                    class="text-primary-purple hover:text-primary-pink mr-3"
                  >
                    Edit
                  </button>
                  <button
        ...
        <p v-else class="text-center py-8 text-sm text-gray-500">No document types found</p>
      </div>
    </div>
