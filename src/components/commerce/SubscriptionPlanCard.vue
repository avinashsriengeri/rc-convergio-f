<template>
  <div class="bg-white border border-gray-200 rounded-2xl shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 ease-in-out min-h-[320px] h-full flex flex-col overflow-hidden">
    <!-- Enhanced Header Section -->
    <div class="bg-gradient-to-br from-blue-50 via-white to-purple-50 p-6 pb-4 relative">
      <div class="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-blue-100 to-purple-100 rounded-full -translate-y-16 translate-x-16 opacity-20"></div>
      <div class="relative">
        <div class="flex items-center justify-between mb-3">
          <h3 class="text-xl font-bold text-gray-900">{{ plan.name }}</h3>
          <div class="flex items-center space-x-2">
            <span v-if="plan.active !== false" class="inline-flex items-center px-3 py-1 text-xs rounded-full font-semibold bg-gradient-to-r from-green-500 to-green-600 text-white shadow-sm">
              <div class="w-2 h-2 bg-white rounded-full mr-1.5"></div>
              Active
            </span>
            <span v-else class="inline-flex items-center px-3 py-1 text-xs rounded-full font-semibold bg-gray-100 text-gray-600">
              Inactive
            </span>
            <div v-if="plan.stripe_price_id" class="flex items-center text-green-600 bg-green-50 p-1.5 rounded-full" title="Synced with Stripe">
        ...
        <p class="text-sm text-gray-600 leading-relaxed">{{ plan.metadata?.description || plan.description || 'No description provided' }}</p>
      </div>
    </div>
      ...
      <div class="space-y-4">
        ...
        <div class="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-4 border border-blue-100">
          <div class="flex items-center justify-between">
            <div class="flex items-center">
              ...
              <div>
                <p class="text-xs font-medium text-gray-600 uppercase tracking-wide">Price</p>
                <p class="text-2xl font-bold text-gray-900">${{ formatPrice(plan.amount_cents ? plan.amount_cents / 100 : plan.price) }}</p>
                <p class="text-xs text-gray-500">{{ plan.currency?.toUpperCase() }} / {{ plan.interval }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="flex items-center p-3 bg-gray-50 rounded-lg">
            ...
            <div>
              <p class="text-xs font-medium text-gray-600">Interval</p>
          ...
          <div class="flex items-center p-3 bg-gray-50 rounded-lg">
            ...
            <div>
              <p class="text-xs font-medium text-gray-600">Trial</p>
              <p class="text-sm font-semibold text-gray-900">{{ plan.trial_days || 0 }} days</p>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
          <div class="flex items-center">
            ...
            <div>
              <p class="text-xs font-medium text-gray-600">Stripe Status</p>
    ...
    <!-- Enhanced Footer Section (Action Buttons) -->
    <div class="mt-6 border-t border-gray-100 pt-6 px-6 pb-6 bg-gradient-to-r from-gray-50 to-white">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center space-x-3">
          <button
            @click="$emit('edit', plan)"
            class="inline-flex items-center px-4 py-2.5 border border-gray-300 rounded-xl text-sm font-semibold text-gray-700 bg-white hover:bg-gray-50 hover:border-gray-400 hover:shadow-md transition-all duration-200"
            title="Edit Plan"
          >
        ...
        <div class="flex items-center space-x-3">
          <button
            @click="deletePlan"
            class="inline-flex items-center px-4 py-2.5 border border-gray-300 rounded-xl text-sm font-semibold text-gray-700 bg-white hover:bg-red-50 hover:text-red-600 hover:border-red-300 hover:shadow-md transition-all duration-200"
            title="Delete Plan"
          >
            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          await window.Swal.fire({
            ...
            html: `
              <div class="text-left">
                ...
                ${isDemoMode ? '<p class="mb-3 text-sm text-blue-600 bg-blue-50 p-2 rounded border">🎯 <strong>Demo Mode:</strong> This is a demo checkout link for testing purposes.</p>' : ''}
