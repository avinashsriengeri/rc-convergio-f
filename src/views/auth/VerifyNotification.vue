      ...
      <div class="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
        ...
        <div class="mt-6 text-center">
          ...
          <div class="space-y-3">
            ...
            <button
              @click="goToSupport"
              class="block w-full text-sm text-gray-600 hover:text-gray-900 font-medium transition-colors"
            >
              Contact Support
            </button>
          </div>
...
const { user, verificationLoading, resendVerification } = useAuth()
...
// Lifecycle
onMounted(() => {
  // Check for temporary user data from login
  const tempUser = localStorage.getItem('temp_user')
  if (tempUser) {
    try {
      const parsedUser = JSON.parse(tempUser)
      // Update the user state with temporary data
