  ...
  <div class="min-h-screen bg-gray-50">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="py-6">
          <div class="flex items-center justify-between">
            <div>
              <h1 class="text-2xl font-bold text-gray-900">Commerce Overview</h1>
              <p class="mt-1 text-sm text-gray-500">Monitor your commerce platform performance and key metrics</p>
            </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatsCard
          title="Total Orders"
          :value="analyticsOverview?.total_orders || stats.total_orders || 0"
          :icon="OrdersIcon"
          icon-bg-class="bg-blue-500"
          icon-class="text-white"
          :change="analyticsOverview?.orders_growth"
          :change-type="analyticsOverview?.orders_growth >= 0 ? 'positive' : 'negative'"
          footer="View all orders"
          footer-link="/commerce/orders"
        />

        <StatsCard
          title="Total Revenue"
          :value="analyticsOverview?.total_revenue || stats.total_revenue || 0"
          :icon="RevenueIcon"
          icon-bg-class="bg-green-500"
          icon-class="text-white"
          format="currency"
          :change="analyticsOverview?.revenue_growth"
          :change-type="analyticsOverview?.revenue_growth >= 0 ? 'positive' : 'negative'"
        />

        <StatsCard
          title="Conversion Rate"
          :value="analyticsOverview?.conversion_rate || 0"
          :icon="LinksIcon"
          icon-bg-class="bg-purple-500"
          icon-class="text-white"
          format="percentage"
          :change="analyticsOverview?.conversion_growth"
          :change-type="analyticsOverview?.conversion_growth >= 0 ? 'positive' : 'negative'"
          footer="View analytics"
        />

        <StatsCard
          title="Avg Order Value"
          :value="analyticsOverview?.average_order_value || stats.average_order_value || 0"
          :icon="AverageIcon"
          icon-bg-class="bg-orange-500"
          icon-class="text-white"
          format="currency"
          :change="analyticsOverview?.aov_growth"
          :change-type="analyticsOverview?.aov_growth >= 0 ? 'positive' : 'negative'"
        />
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatsCard
          ...
          :value="subscriptionsStore.analytics.activeSubscriptions"
          :icon="SubscriptionIcon"
          icon-bg-class="bg-indigo-500"
          icon-class="text-white"
          :change="subscriptionsStore.analytics.subscription_growth"
          :change-type="subscriptionsStore.analytics.subscription_growth >= 0 ? 'positive' : 'negative'"
          footer="View subscriptions"
          footer-link="/commerce/subscriptions"
        />

        <StatsCard
          title="Monthly Recurring Revenue"
          :value="subscriptionsStore.analytics.monthlyRecurringRevenue"
          :icon="MRRIcon"
          icon-bg-class="bg-emerald-500"
          icon-class="text-white"
          format="currency"
          :change="subscriptionsStore.analytics.mrr_growth"
          :change-type="subscriptionsStore.analytics.mrr_growth >= 0 ? 'positive' : 'negative'"
        />

        <StatsCard
          title="Churn Rate"
          :value="subscriptionsStore.analytics.churnRate"
          :icon="ChurnIcon"
          icon-bg-class="bg-red-500"
          icon-class="text-white"
          format="percentage"
          :change="subscriptionsStore.analytics.churn_growth"
          :change-type="subscriptionsStore.analytics.churn_growth <= 0 ? 'positive' : 'negative'"
        />

        <StatsCard
          title="Revenue This Month"
          :value="subscriptionsStore.analytics.revenueThisMonth"
          :icon="RevenueIcon"
          icon-bg-class="bg-teal-500"
          icon-class="text-white"
          format="currency"
          :change="subscriptionsStore.analytics.monthly_revenue_growth"
          :change-type="subscriptionsStore.analytics.monthly_revenue_growth >= 0 ? 'positive' : 'negative'"
        />
      </div>
        ...
        <div class="bg-white shadow rounded-lg">
          <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
              ...
              <router-link
                to="/commerce/orders"
                class="text-sm text-primary-purple hover:text-primary-purple-600"
              >
                View all
              </router-link>
            </div>
