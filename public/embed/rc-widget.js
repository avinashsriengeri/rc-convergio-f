...
(function() {
  ...
  function openWidget() {
    ...
    // Set iframe source without exposing tenant ID in URL
    const helpUrl = `${widgetConfig.widgetUrl}/widget?modules=${widgetConfig.modules.join(',')}&embed=true`;
    widgetIframe.src = helpUrl;
    
    // Set up secure communication with iframe
  ...
  function setupIframeCommunication() {
    ...
    window.addEventListener('message', function(event) {
      ...
      if (event.data.type === 'REQUEST_TENANT_ID') {
        // Securely send tenant ID to iframe
        widgetIframe.contentWindow.postMessage({
          type: 'TENANT_ID_RESPONSE',
          tenantId: widgetConfig.tenant
        }, widgetConfig.widgetUrl);
      }
